...
export default function KoordinatorDashboard() {
  const { dashboardData, isLoadingDashboard, dashboardError, refetchDashboard } = useKoordinatorDashboard();
  ...
  if (dashboardError) {
    return (
      <div className="space-y-4">
        <ErrorMessage message={`Failed to load dashboard: ${dashboardError.message}`} />
  ...
  const { user_context, eschool, statistics, recent_activities, multi_role_insights } = dashboardData;

  return (
    <div className="space-y-6">
      {/* Header */}
      <div className="flex flex-col space-y-2">
        <h1 className="text-3xl font-bold tracking-tight">Koordinator Dashboard</h1>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">{statistics.attendance_rate.toFixed(1)}%</div>
            <p className="text-xs text-muted-foreground">
              {statistics.present_sessions} of {statistics.total_sessions} sessions
            </p>
          </CardContent>
        </Card>
        ...
        <Card>
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
            <CardTitle className="text-sm font-medium">Kas Collection</CardTitle>
            <DollarSign className="h-4 w-4 text-muted-foreground" />
          </CardHeader>
          <CardContent>
            <div className="text-2xl font-bold">{statistics.kas_collection_rate.toFixed(1)}%</div>
            <p className="text-xs text-muted-foreground">
        ...
        <Card>
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
            <CardTitle className="text-sm font-medium">Outstanding Kas</CardTitle>
            <AlertCircle className="h-4 w-4 text-muted-foreground" />
          </CardHeader>
          <CardContent>
            <div className="text-2xl font-bold">Rp {statistics.outstanding_kas.toLocaleString()}</div>
            <p className="text-xs text-muted-foreground">
          ...
          <CardContent>
            <div className="space-y-4">
              <div className="flex items-center justify-between">
                ...
                <Badge variant="default">{statistics.role_breakdown.koordinator}</Badge>
              </div>
              <div className="flex items-center justify-between">
                <span className="text-sm">Bendahara</span>
                <Badge variant="secondary">{statistics.role_breakdown.bendahara}</Badge>
              </div>
              <div className="flex items-center justify-between">
                <span className="text-sm">Members</span>
                <Badge variant="outline">{statistics.role_breakdown.member}</Badge>
              </div>
            </div>
          </CardContent>
        </Card>
          ...
          <CardContent>
            <div className="space-y-4">
              ...
              <div>
                ...
                <p className="text-lg font-semibold">Rp {eschool.monthly_kas_amount.toLocaleString()}</p>
              </div>
            </div>
          </CardContent>
        </Card>
                ...
                recent_activities.recent_attendance.map((record) => (
                  <div key={record.id} className="flex items-center justify-between border-b pb-2">
                    <div className="flex items-center space-x-3">
                      ...
                      <div>
                        <p className="text-sm font-medium">{record.member_name}</p>
                        <p className="text-xs text-muted-foreground">
                          {new Date(record.date).toLocaleDateString()} • {record.role_in_eschool}
                        </p>
                      </div>
                    </div>
                  ...
                  <div key={index} className="flex items-center justify-between border-b pb-2">
                    <div className="flex items-center space-x-3">
                      ...
                      <div>
                        ...
                        <p className="text-xs text-muted-foreground">
                          {new Date(payment.paid_date).toLocaleDateString()} • {payment.role_in_eschool}
                        </p>
                      </div>
                    </div>
                    <div className="text-right">
                      <p className="text-sm font-medium">Rp {payment.amount.toLocaleString()}</p>
                      <p className="text-xs text-muted-foreground">Approved by {payment.approved_by}</p>
                    </div>
                  </div>
                ))
              ) : (
      ...
      {multi_role_insights.users_with_multiple_roles.length > 0 && (
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              Members participating in multiple eschools ({multi_role_insights.cross_participation_rate.toFixed(1)}% cross-participation rate)
            </CardDescription>
          </CardHeader>
          <CardContent>
            <div className="space-y-4">
              {multi_role_insights.users_with_multiple_roles.map((user) => (
                <div key={user.user_id} className="flex items-center justify-between border-b pb-2">
                  ...
                  <Badge variant="secondary">{user.roles_in_school.length} roles</Badge>
                </div>
              ))}
            </div>
          </CardContent>

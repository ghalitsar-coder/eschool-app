...
const StudentDashboard: React.FC = () => {
  ...
  // Handle eschool change from role switcher
  const handleEschoolChange = (eschoolId: number, role: string) => {
    setSelectedEschoolId(eschoolId);
    // Update the active tab to the selected eschool
    setActiveTab(`eschool-${eschoolId}`);
  };
                      ...
                      {profileData.eschool_roles?.slice(0, 4).map((role) => (
                        <div 
                          ...
                          onClick={() => {
                            ...
                            setActiveTab(`eschool-${role.eschool_id}`);
                          }}
                        >
                          <div>
                            <p className="font-medium">{role.eschool_name}</p>
                            <Badge variant="secondary" className="mt-1">
                              {role.role_in_eschool}
                            </Badge>
                          </div>
